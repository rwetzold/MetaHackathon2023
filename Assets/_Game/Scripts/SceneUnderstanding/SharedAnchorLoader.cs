...
namespace Hackathon
{
    public class SharedAnchorLoader : MonoBehaviour
    {
        ...
        {
            SampleController.Instance.Log("OnSpaceQueryComplete: requestId: " + requestId + " isSuccessful: " +
                                          isSuccessful);
            ...
            foreach (var queryResult in queryResults)
            {
                ...
                {
                    NetworkManager.Instance.SessionStart();
        ...
        public void LoadLocalAnchors()
        {
            ...
            {
                SampleController.Instance.Log("LoadLocalAnchors: successfully queried local anchors requestId: " +
                                              requestId);
        ...
        public void LoadLastUsedCachedAnchor()
        {
            ...
            if (didSuccessfullyQuerySpaces)
            {
                SampleController.Instance.Log(
                    "LoadLastUsedCachedAnchor: successfully queried local anchors requestId: " + requestId);
        ...
        private void LoadLocalAnchorsFromRemote(HashSet<Guid> uuids)
        {
            ...
            {
                SampleController.Instance.Log(
                    "LoadLocalAnchorsFromRemote: successfully queried local anchors requestId: " + requestId);
        ...
        private void LoadCloudAnchorsFromRemote(HashSet<Guid> uuids)
        {
            ...
            {
                SampleController.Instance.Log(
                    "LoadCloudAnchorsFromRemote: successfully queried cloud anchors: numSpaces: " + uuids.Count +
                    " requestId: " + requestId);
                queryMode = AnchorQueryMode.CLOUD;
            }
            else
            {
                SampleController.Instance.Log("LoadCloudAnchorsFromRemote: failed to query cloud anchors: numSpaces: " +
                                              uuids.Count);
            }
        }
    }
