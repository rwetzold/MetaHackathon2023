...
namespace Hackathon
{
    public class CachedSharedAnchor : MonoBehaviour
    {
        ...
        [SerializeField] Image sharedAnchorImage;
        ...
        {
            var userIds = NetworkManager.GetUserList().Select(userId => userId.ToString()).ToArray();
            if (userIds.Length == 0)
            {
        ...
        public void ShareAnchor()
        {
            ...
            _spatialAnchor.Save(saveOptions, (spatialAnchor, isSuccessful) =>
            {
                if (isSuccessful)
                {
                    ...
                    foreach (string strUsername in NetworkManager.GetUsers())
                    {
                        spaceUserList.Add(new OVRSpaceUser(ulong.Parse(strUsername)));
                    }

                    OVRSpatialAnchor.Share(new List<OVRSpatialAnchor> { spatialAnchor }, spaceUserList,
                        OnShareComplete);
                }
                else
                {
            ...
            OVRSpatialAnchor.OperationResult result)
        {
            ...
            foreach (var spatialAnchor in spatialAnchors)
            {
                SampleController.Instance.Log("OnShareComplete: space: " + spatialAnchor.Space.Handle + ", uuid: " +
                                              spatialAnchor.Uuid);
            ...
            NetworkManager.Instance.PublishAnchorUuids(uuids, (uint)uuids.Length, true);
        }
        ...
        public void SendLocalAnchor()
        {
            ...
            NetworkManager.Instance.PublishAnchorUuids(uuids, (uint)uuids.Length, false);
        }
