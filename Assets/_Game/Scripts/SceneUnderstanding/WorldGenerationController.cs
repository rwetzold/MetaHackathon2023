...
namespace Hackathon
{
    public class WorldGenerationController : MonoBehaviour
    {
        ...
        private GameObject PopulateScaledObstacle(Obstacle obstacle, ObstacleType type)
        {
            ...
            Quaternion toUp = Quaternion.AngleAxis(-Vector3.Angle(obstacleRotation * Vector3.up, Vector3.up),
                obstacle.rotation * Vector3.right);
            obstacleRotation = toUp * obstacleRotation;
            Vector3 obstacleScale;

            float dotXY =
                Mathf.Abs(Vector3.Dot(Vector3.Cross(obstacle.rotation * Vector3.right, obstacle.rotation * Vector3.up),
                    Vector3.up));
            float dotXZ =
                Mathf.Abs(Vector3.Dot(
                    Vector3.Cross(obstacle.rotation * Vector3.right, obstacle.rotation * Vector3.forward),
                    Vector3.up));
            float dotYZ =
                Mathf.Abs(Vector3.Dot(
                    Vector3.Cross(obstacle.rotation * Vector3.up, obstacle.rotation * Vector3.forward),
                    Vector3.up));

            if (dotXY > dotXZ && dotXY > dotYZ)
            {
                obstacleScale = new Vector3(obstacle.boundingBox.size.x, obstacle.boundingBox.size.z,
                    obstacle.boundingBox.size.y);
            }
            else if (dotXZ > dotXY && dotXZ > dotYZ)
            {
                obstacleScale = new Vector3(obstacle.boundingBox.size.x, obstacle.boundingBox.size.y,
                    obstacle.boundingBox.size.z);
            }
            else
            {
                obstacleScale = new Vector3(obstacle.boundingBox.size.y, obstacle.boundingBox.size.x,
                    obstacle.boundingBox.size.z);
            }
