...
namespace Hackathon
{
    ...
    {
        private static readonly Dictionary<string, ObstacleType> _obstacleTypeMap =
            new Dictionary<string, ObstacleType>()
            {
        ...
        [SerializeField] WorldGenerationController worldGenerationController;
        ...
        {
            Debug.Log("SceneApiSceneCaptureStrategy::CaptureScene - Scene Capture Started");
        ...
        {
            Debug.Log("PhotonAnchorManager::OnSceneCreated - Scene Capture Complete");
        }

        private void Cleanup()
        {
            Debug.Log("Unsubscribing all OVRManager listeners");
            OVRManager.SceneCaptureComplete -= OnSceneCaptureComplete;
            OVRManager.SpaceQueryComplete -= OnSpaceQueryCompleted;
            OVRManager.SpaceSetComponentStatusComplete -= OnSpaceSetComponentStatusComplete;
        }
        ...
        private void StartRoomCapture()
        {
            ...
            Debug.Log("requesting scene capture");
            var success = OVRPlugin.RequestSceneCapture(req, out id);
            if (!success)
            {
                Debug.Log("Failure requesting scene capture");
            }
        }
        ...
        {
            Debug.Log("Scene capture complete");
            LoadRoomLayout();
        }
        ...
        public void LoadRoomLayout()
        {
            ...
            Debug.Log("Starting Room Layout query");
        ...
        private void LoadEntitiesById(List<Guid> ids)
        {
            ...
            Debug.Log(
                $"Starting entity ID query for [{numIds}] ids: [{string.Join(", ", idInfo.Ids.Select(uuid => uuid.ToString()))}]");
        ...
        private void AddEntityToRoom(ulong space)
        {
            ...
            OVRPlugin.GetSpaceComponentStatus(space, OVRPlugin.SpaceComponentType.Locatable, out var locatableEnabled,
                out _);
            if (!locatableEnabled)
            {
            ...
            OVRPlugin.GetSpaceComponentStatus(space, OVRPlugin.SpaceComponentType.Bounded2D, out var bounded2dEnabled,
                out _);
            Debug.Log($"{MethodBase.GetCurrentMethod().Name} space: [{space}] bounded2dEnabled [{bounded2dEnabled}]");

            OVRPlugin.GetSpaceComponentStatus(space, OVRPlugin.SpaceComponentType.Bounded3D, out var bounded3dEnabled,
                out _);
        ...
        private static Obstacle CreateObstacle(OVRPose worldPose, OVRPlugin.Rectf rectf, string labels,
        ...
        private static Obstacle CreateObstacle(OVRPose worldPose, OVRPlugin.Boundsf boundsf, string labels,
            ObstacleType obstacleType)
        {
            var position = worldPose.position;
            position.y /= 2.0f;
        ...
        private bool EnableComponentIfNecessary(ulong space, OVRPlugin.SpaceComponentType componentType)
        {
            ...
            {
                Debug.Log(
                    $"{MethodBase.GetCurrentMethod().Name} component [{componentType}] is already enabled for space [{space}]");
                return false;
            }
            ...
            Debug.Log(
                $"{MethodBase.GetCurrentMethod().Name} component [{componentType}] requested for space [{space}] with requestId [{requestId}]");
            _componentEnablingInProgress++;
            Debug.Log($"_componentEnablingInProgress now at [{_componentEnablingInProgress}]");
            return true;
        }

        private void OnSpaceSetComponentStatusComplete(ulong requestId, bool result, OVRSpace space, Guid id,
            OVRPlugin.SpaceComponentType componentType, bool enabled)
        {
            Debug.Log(
                $"{MethodBase.GetCurrentMethod().Name} requestId [{requestId}] for space [{space}] result [{result}]");
            _componentEnablingInProgress--;
            Debug.Log($"_componentEnablingInProgress now at [{_componentEnablingInProgress}]");
